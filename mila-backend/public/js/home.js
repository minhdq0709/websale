...
function renderProductCard(product) {
  ...
  return `
    <div class="bg-white rounded-xl product-card-shadow group flex flex-col h-full transition-all hover:-translate-y-2 overflow-hidden cursor-pointer shadow-sm border border-outline-variant hover:border-primary" onclick="goToDetail(${product.id})">
      ...
      <div class="flex flex-col flex-grow p-6">
        ...
        <div class="mt-auto pt-2">
          ...
          <div class="flex items-center gap-3" onclick="event.stopPropagation();">
            ...
            <button class="w-12 h-12 border border-outline-variant text-primary rounded-lg flex items-center justify-center hover:bg-surface-container-low transition-colors active:scale-90 duration-100 shrink-0" 
...
async function loadFeaturedProducts() {
  ...
  // Render product cards using helper
...
/**
 * Tai danh sach danh muc san pham nhanh o trang chu
 */
async function loadQuickCategories() {
  ...
  container.innerHTML = categories.map(cat => `
    <a href="/products.html?category=${encodeURIComponent(cat.slug)}" 
      ...
      <span class="font-bold text-on-surface text-body-sm group-hover:text-primary transition-colors">${cat.name}</span>
    </a>
  `).join('');
}

/**
 * Load products grouped by categories and append sections to the home page
 */
async function loadCategoryGroupedProducts() {
  const container = document.getElementById('home-category-products-container');
  if (!container) return;

  // Render skeletons for categories sections
